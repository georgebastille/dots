...
# Steps:
# 0) Disable SIP (rootless) mode
# 0.1) Reboot
# 0.2) Hold Cmd+R immediately to boot into recovery mode
# 0.3) Utilities > Terminal
# 0.4) $ csrutil disable
# 0.5) Reboot
# 1) Connect only the external monitors (close MBP lid while connected to power)
# 2) $ cd /tmp
# 3) $ ruby patch-edid.rb
# 4) When you `ls`, you will see a new directory containing the configuration
#    information.
# 5) Move directory to:
#    /System/Library/Displays/Contents/Resources/Overrides
# 6) Re-enable SIP (rootless) mode
# 6.1) Reboot
# 6.2) Hold Cmd+R immediately to boot into recovery mode
# 6.3) Utilities > Terminal
# 6.4) $ csrutil enable
# 6.5) Reboot
#
# Display should look better now
